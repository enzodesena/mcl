...
public:
  RandomGenerator() noexcept
    ...
    , distribution_uniform_(std::uniform_real_distribution<double>(0.0, 1.0)) { generator_.seed(1u); }

  RandomGenerator(
    unsigned int seed) noexcept
    : distribution_norm_(std::normal_distribution<double>(0.0, 1.0))
    , distribution_uniform_(std::uniform_real_distribution<double>(0.0, 1.0)) { generator_.seed(seed); }
    ...
    const Int size) noexcept
  {
    ...
    for (Int i = 0; i < size; i++) { output[i] = distribution_norm_(generator_); }
    return output;
  }
  ...
  {
    Vector<Real> output(size);
    for (Int i = 0; i < size; i++) { output[i] = distribution_uniform_(generator_); }
    return output;
  }
   ...
   */
  Real Rand() noexcept { return Rand(1)[0]; }
  ...
  void SetSeed(
    unsigned int seed) noexcept { generator_.seed(seed); }
