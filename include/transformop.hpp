 ...
 */
template<typename T>
Vector<Complex<T>> Rfft(
  const Vector<T>& input,
  size_t n_point) noexcept
{
  return Elements
  (
    Fft(CastToComplex(input), n_point),
    0,
