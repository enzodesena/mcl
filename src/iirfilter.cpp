...
void IirFilter::ProcessBlock(std::span<const Real> input_data, std::span<Real> output_data) noexcept {
  if (B_.size() == 1) {
...
void IirFilterBank::ProcessSample(const Real input, std::span<Real> output_data) {
